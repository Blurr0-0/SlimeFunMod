...
public class ShapedEnhancedCraftingRecipe implements EnhancedCraftingRecipe, Recipe<CraftingContainer> {
    ...
    public ShapedEnhancedCraftingRecipe(ResourceLocation id, String group, int width, int height,
    ...
    public static class Type implements RecipeType<ShapedEnhancedCraftingRecipe> {
        public static final String ID = "enhanced_crafting_shaped";
    }
    ...
    static String[] patternFromJson(JsonArray array) {
        ...
        if (astring.length > EnhancedCraftingTableMenu.height) {
            throw new JsonSyntaxException(
                    "Invalid pattern: too many rows, " + EnhancedCraftingTableMenu.height + " is maximum");
        } else if (astring.length == 0) {
            throw new JsonSyntaxException("Invalid pattern: empty pattern not allowed");
        } else {
            for (int i = 0; i < astring.length; ++i) {
                String s = GsonHelper.convertToString(array.get(i), "pattern[" + i + "]");
                if (s.length() > EnhancedCraftingTableMenu.width) {
                    throw new JsonSyntaxException(
                            "Invalid pattern: too many columns, " + EnhancedCraftingTableMenu.width + " is maximum");
                }
    ...
    public static class Serializer implements RecipeSerializer<ShapedEnhancedCraftingRecipe> {
        public static final Serializer INSTANCE = new Serializer();
        public static final ResourceLocation ID = new ResourceLocation(SlimeFun.MOD_ID, "enhanced_crafting_shaped");

        @Override
        public ShapedEnhancedCraftingRecipe fromJson(ResourceLocation id, JsonObject json) {
            String s = GsonHelper.getAsString(json, "group", "");
            Map<String, Ingredient> ingredientMap = ShapedEnhancedCraftingRecipe
                    .keyFromJson(GsonHelper.getAsJsonObject(json, "key"));
            String[] patternArray = ShapedEnhancedCraftingRecipe
                    .shrink(patternFromJson(GsonHelper.getAsJsonArray(json, "pattern")));
            int i = patternArray[0].length();
            int j = patternArray.length;
            NonNullList<Ingredient> dissolvePattern = ShapedEnhancedCraftingRecipe.dissolvePattern(patternArray, ingredientMap, i, j);
            ItemStack output = ShapedEnhancedCraftingRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(json, "result"));

            return new ShapedEnhancedCraftingRecipe(id, s, i, j, dissolvePattern, output);
        }

        @Override
        public ShapedEnhancedCraftingRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
            ...
            return new ShapedEnhancedCraftingRecipe(id, s, i, j, nonnulllist, itemstack);
        }

        @Override
        public void toNetwork(FriendlyByteBuf buf, ShapedEnhancedCraftingRecipe recipe) {
